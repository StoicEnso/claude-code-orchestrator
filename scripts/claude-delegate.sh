...
usage() {
  cat <<'EOF'
Usage:
  claude-delegate.sh dispatch <profile> <budget|none> <model> <label> "<task>" [cc-profile options]
  claude-delegate.sh poll <task-id>
  claude-delegate.sh result [--text|--raw] <task-id>
  claude-delegate.sh resume <task-id> <budget|none> "<follow-up>" [resume options]
